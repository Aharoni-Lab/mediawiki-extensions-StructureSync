...
On init, call `semanticschemas-hierarchy` API to get tree data. The hierarchy API requires a `category` parameter -- use the root category. To find the root: load the tree for any category -- the API returns `rootCategory` in the response. Alternatively, the PHP SpecialPage can embed the root category as a data attribute on the tree container: `data-root-category="..."`.

Better approach: Have the PHP `SpecialCreateSemanticPage::renderLayout()` query WikiCategoryStore to find the root category (a category with no parents) and embed it as `data-root-category` on `#ss-createpage-tree`. The JS reads this attribute.

IMPORTANT: If the PHP class from Plan 02 does not embed the root category, the JS should handle this by calling the hierarchy API with an empty/first category and reading `rootCategory` from the response. But prefer the data attribute approach.
...
**Init function:**
1. Read root category from `$('#ss-createpage-tree').data('root-category')` or fall back to hierarchy API
2. Fetch hierarchy tree data
...
9. Click submit -- verify redirect to Special:FormEdit with the correct composite form
10. Test edge cases: no categories selected (empty state shown), rapid checking/unchecking (no stale previews)
  </how-to-verify>
...
3. Tree renders from hierarchy API data
...
7. Submit generates form server-side and redirects to FormEdit
